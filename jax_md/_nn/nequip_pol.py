...
class NequIPEnergyModel(nn.Module):
  ...
  def __call__(self, graph):
    ...
    h_node_mag = Linear(irreps_out=second_to_final_irreps)(h_node)
    magnetizations = Linear(irreps_out=final_irreps_mag)(h_node_mag).array    
    magnetizations = self.scale_occ[...,jnp.newaxis] * magnetizations[:-1] + self.shift_occ[...,jnp.newaxis]
...
def model_from_config(cfg: ConfigDict
  ...
  shift, scale, shift_occ, scale_occ = nn_util.get_shift_and_scale_occ(cfg)
