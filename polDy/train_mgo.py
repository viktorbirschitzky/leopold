...
def make_config(energy, force, occ, atoms, distinguish_charge_state=False):
    ...
    indices = np.maximum.reduceat(
        nonzero_indices[1], np.unique(nonzero_indices[0], return_index=True)[1]
    )
...
train_box, train_position, train_energy, train_force, train_atoms, train_occ = (
    train_data
)
...
with open("standard_model.yaml", "r") as f:
    f.write(yaml.dump(config.to_yaml()))
...
neighbor_list, init_fn, energy_fn = energy.nequip_neighbor_list(
    displacement, train_box, config, fractional_coordinates=True, disable_cell_list=True
)
...
def en_occ_n_nbrs(params, box, position, atoms):
    ...
    predicted_energy, predicted_occs = energy_fn(
        params, position, _nbrs, box=box, atoms=atoms
    )
...
def occ_loss_fn(params, box, position, occs, atoms):
    ...
    return jnp.mean(
        (predicted_occs[..., 0] - occs[..., 0]) ** 2
        + (predicted_occs[..., 1] - occs[..., 1]) ** 2
        + (
            (predicted_occs[..., 0] + predicted_occs[..., 1])
            - (occs[..., 0] + occs[..., 1])
        )
        ** 2
        + (
...
def loss_fn(params, box, position, energy, force, occs, atoms):
    ...
    return (
        energy_loss_fn(params, box, position, energy, atoms)
...
params = init_fn(
    key, jnp.array(test_position)[0, 0], neighbor=nbrs, atoms=test_atoms[0][0]
)
...
for i in range(5000):
    if i % 1 == 0:
        ...
        print(f"Epoch {i}; Train Loss: {train_loss}. " f"Test Loss: {val_loss}")
    ...
    for p, e, f, o, a in zip(
        train_position, train_energy, train_force, train_occ, train_atoms
    ):
